...
export class ShopPage {
    ...
    return `
      ...
      <div class="space-y-3 pt-4 border-t border-white/10">
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs font-mono font-bold text-[#d4af37]" id="${prefix}-price-val">Rs. ${f.maxPrice.toLocaleString()}</span>
        </div>
        <input 
          type="range" 
  ...
  static renderActiveFilterPills() {
    ...
    if (f.maxPrice < 60000) pills.push({ key: "maxPrice", label: `Under Rs. ${f.maxPrice.toLocaleString()}` });
    ...
    ["desktop", "mobile"].forEach(prefix => {
      ...
      if (priceSlider) {
        priceSlider.addEventListener("input", () => {
          ...
          if (priceVal) priceVal.textContent = `Rs. ${val.toLocaleString()}`;
