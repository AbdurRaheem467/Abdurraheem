...
export class CartDrawer {
      ...
      drawer.innerHTML = `
        <div class="fixed inset-y-0 right-0 max-w-md w-full bg-[#0f0f13] border-l border-[#d4af37]/30 flex flex-col justify-between shadow-[0_0_50px_rgba(0,0,0,0.9)] transform translate-x-full transition-transform duration-300" id="cart-drawer-panel">
          ...
          <div class="p-6 border-b border-white/10 flex items-center justify-between bg-black/40">
            ...
            <button id="cart-drawer-close-btn" class="p-2 text-zinc-400 hover:text-white rounded-lg hover:bg-white/5 transition-colors" aria-label="Close Shopping Bag">
              <i data-lucide="x" class="w-5 h-5"></i>
            </button>
          </div>

          <!-- Items List -->
          <div id="cart-drawer-items" class="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
          ...
          <div class="p-6 border-t border-white/10 bg-black/60 space-y-4">
            ...
            <div class="space-y-1.5 text-xs font-mono">
              <div class="flex justify-between text-zinc-400">
                <span>Subtotal</span>
                <span id="cart-drawer-subtotal" class="text-white font-bold">$0</span>
              </div>
              <div class="flex justify-between text-zinc-400">
                <span>Shipping</span>
              ...
              <div class="flex justify-between text-sm text-white pt-2 border-t border-white/10">
                ...
                <span id="cart-drawer-total" class="font-bold text-[#d4af37] text-base">$0</span>
              </div>
            </div>
  ...
  static renderItems() {
    ...
    container.innerHTML = items.map(item => {
      const itemFormattedPrice = new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(item.price);
      return `
