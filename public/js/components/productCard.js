...
export class ProductCard {
  static render(product, options = {}) {
    ...
    const formattedOriginalPrice = product.originalPrice ? new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(product.originalPrice) : null;
    
    const secondImage = (product.images && product.images.length > 1) ? product.images[1] : product.images[0];
    ...
    return `
      <div class="product-card group relative bg-gradient-to-b from-[#141418] to-[#0f0f12] rounded-2xl border border-white/10 hover:border-[#d4af37]/50 shadow-xl transition-all duration-500 flex flex-col justify-between overflow-hidden hover:shadow-[0_15px_40px_rgba(0,0,0,0.6)]" data-product-id="${product.id}">
        ...
        <div class="relative w-full aspect-square overflow-hidden bg-[#18181c] p-6 flex items-center justify-center">
          ...
          <!-- Product Image with Secondary Image Hover Transition -->
          <a href="#product-${product.id}" class="block w-full h-full relative cursor-pointer group-hover:scale-105 transition-transform duration-700">
            <img 
              src="${product.images[0]}" 
              alt="${product.name}" 
              loading="lazy" 
              class="product-img-primary w-full h-full object-contain filter drop-shadow-[0_10px_20px_rgba(0,0,0,0.7)] transition-opacity duration-500 ${product.images.length > 1 ? 'group-hover:opacity-0' : ''}"
            />
            ${product.images.length > 1 ? `
              <img 
                ...
                loading="lazy" 
                class="product-img-secondary absolute inset-0 w-full h-full object-contain filter drop-shadow-[0_10px_20px_rgba(0,0,0,0.7)] opacity-0 group-hover:opacity-100 transition-opacity duration-500"
              />
            ` : ''}
          </a>
        ...
        <div class="p-5 flex flex-col flex-1 justify-between bg-gradient-to-b from-transparent to-black/40">
          ...
          <div class="pt-3 border-t border-white/10 flex items-center justify-between gap-3">
            <div>
              <div class="text-xs text-zinc-500 line-through ${hasDiscount ? 'block' : 'hidden'}">
            ...
            <button 
              class="add-to-cart-btn px-4 py-2.5 rounded-xl bg-white/10 hover:bg-[#d4af37] text-white hover:text-black font-semibold text-xs uppercase tracking-wider border border-white/10 hover:border-[#d4af37] flex items-center gap-1.5 transition-all active:scale-95 shadow-md shrink-0" 
              data-product-id="${product.id}"
  ...
  static setupCardEvents(container = document) {
    ...
    container.querySelectorAll(".add-to-cart-btn").forEach(btn => {
      btn.addEventListener("click", (e) => {
        ...
        const productId = btn.getAttribute("data-product-id");
        if (productId) {
          store.addToCart(productId, 1, true);
        }
      });
    });
